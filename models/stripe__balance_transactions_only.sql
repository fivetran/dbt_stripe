with balance_transaction as (
    ...
    select c.*, nvl(i.created_at, c.created_at) as invoice_created_date, i.status as invoice_status
    from {{ var('charge')}} c
    left join  {{ var('invoice') }} i on i.invoice_id = c.invoice_id
...
select 
    ...
    charge.invoice_created_date as revenue_recognition_date,
    charge.invoice_id,
