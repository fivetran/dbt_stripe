...
select
    ...
    {{ 'coalesce(daily_account_balance_transactions.total_daily_sales_amount / 100.0, 0)' if not var('stripe__convert_values', false) else 'coalesce(daily_account_balance_transactions.total_daily_sales_amount, 0)' }} as total_daily_sales_amount,
    {{ 'coalesce(daily_account_balance_transactions.total_daily_refunds_amount / 100.0, 0)' if not var('stripe__convert_values', false) else 'coalesce(daily_account_balance_transactions.total_daily_refunds_amount, 0)' }} as total_daily_refunds_amount,
    {{ 'coalesce(daily_account_balance_transactions.total_daily_adjustments_amount / 100.0, 0)' if not var('stripe__convert_values', false) else 'coalesce(daily_account_balance_transactions.total_daily_adjustments_amount, 0)' }} as total_daily_adjustments_amount,
    {{ 'coalesce(daily_account_balance_transactions.total_daily_other_transactions_amount / 100.0, 0)' if not var('stripe__convert_values', false) else 'coalesce(daily_account_balance_transactions.total_daily_other_transactions_amount, 0)' }} as total_daily_other_transactions_amount,
    {{ 'coalesce(daily_account_balance_transactions.total_daily_gross_transaction_amount / 100.0, 0)' if not var('stripe__convert_values', false) else 'coalesce(daily_account_balance_transactions.total_daily_gross_transaction_amount, 0)' }} as total_daily_gross_transaction_amount,
    {{ 'coalesce(daily_account_balance_transactions.total_daily_net_transactions_amount / 100.0, 0)' if not var('stripe__convert_values', false) else 'coalesce(daily_account_balance_transactions.total_daily_net_transactions_amount, 0)' }} as total_daily_net_transactions_amount,
    {{ 'coalesce(daily_account_balance_transactions.total_daily_payout_fee_amount / 100.0, 0)' if not var('stripe__convert_values', false) else 'coalesce(daily_account_balance_transactions.total_daily_payout_fee_amount, 0)' }} as total_daily_payout_fee_amount,
    {{ 'coalesce(daily_account_balance_transactions.total_daily_gross_payout_amount / 100.0, 0)' if not var('stripe__convert_values', false) else 'coalesce(daily_account_balance_transactions.total_daily_gross_payout_amount, 0)' }} as total_daily_gross_payout_amount,
    {{ 'coalesce(daily_account_balance_transactions.daily_net_activity_amount / 100.0, 0)' if not var('stripe__convert_values', false) else 'coalesce(daily_account_balance_transactions.daily_net_activity_amount, 0)' }} as daily_net_activity_amount,
    {{ 'coalesce((daily_account_balance_transactions.daily_net_activity_amount + daily_account_balance_transactions.total_daily_gross_payout_amount) / 100.0, 0)' if not var('stripe__convert_values', false) else 'coalesce((daily_account_balance_transactions.daily_net_activity_amount + daily_account_balance_transactions.total_daily_gross_payout_amount), 0)' }} as daily_end_balance_amount,
    ...
    {{ 'coalesce(daily_failed_charges.total_daily_failed_charge_amount / 100.0, 0)' if not var('stripe__convert_values', false) else 'coalesce(daily_failed_charges.total_daily_failed_charge_amount, 0)' }} as total_daily_failed_charge_amount
