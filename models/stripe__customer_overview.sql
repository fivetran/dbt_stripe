with balance_transaction_joined as (
    ...
    select
      ...
      {{ 'coalesce(transactions_grouped.total_sales, 0)' if not var('stripe__convert_values', false) else 'coalesce(transactions_grouped.total_sales, 0)' }} as total_sales,
      {{ 'coalesce(transactions_grouped.total_refunds, 0)' if not var('stripe__convert_values', false) else 'coalesce(transactions_grouped.total_refunds, 0)' }} as total_refunds,
      {{ 'coalesce(transactions_grouped.total_gross_transaction_amount, 0)' if not var('stripe__convert_values', false) else 'coalesce(transactions_grouped.total_gross_transaction_amount, 0)' }} as total_gross_transaction_amount,
      {{ 'coalesce(transactions_grouped.total_fees, 0)' if not var('stripe__convert_values', false) else 'coalesce(transactions_grouped.total_fees, 0)' }} as total_fees,
      {{ 'coalesce(transactions_grouped.total_net_transaction_amount, 0)' if not var('stripe__convert_values', false) else 'coalesce(transactions_grouped.total_net_transaction_amount, 0)' }} as total_net_transaction_amount,
      ...
      {{ 'coalesce(transactions_grouped.sales_this_month, 0)' if not var('stripe__convert_values', false) else 'coalesce(transactions_grouped.sales_this_month, 0)' }} as sales_this_month,
      {{ 'coalesce(transactions_grouped.refunds_this_month, 0)' if not var('stripe__convert_values', false) else 'coalesce(transactions_grouped.refunds_this_month, 0)' }} as refunds_this_month,
      {{ 'coalesce(transactions_grouped.gross_transaction_amount_this_month, 0)' if not var('stripe__convert_values', false) else 'coalesce(transactions_grouped.gross_transaction_amount_this_month, 0)' }} as gross_transaction_amount_this_month,
      {{ 'coalesce(transactions_grouped.fees_this_month, 0)' if not var('stripe__convert_values', false) else 'coalesce(transactions_grouped.fees_this_month, 0)' }} as fees_this_month,
      {{ 'coalesce(transactions_grouped.net_transaction_amount_this_month, 0)' if not var('stripe__convert_values', false) else 'coalesce(transactions_grouped.net_transaction_amount_this_month, 0)' }} as net_transaction_amount_this_month,
    ...
    select
      ...
      {{ 'coalesce(failed_charges_by_customer.total_failed_charge_amount, 0)' if not var('stripe__convert_values', false) else 'coalesce(failed_charges_by_customer.total_failed_charge_amount, 0)' }} as total_failed_charge_amount,
      coalesce(failed_charges_by_customer.failed_charge_count_this_month, 0) as failed_charge_count_this_month,
      {{ 'coalesce(failed_charges_by_customer.failed_charge_amount_this_month, 0)' if not var('stripe__convert_values', false) else 'coalesce(failed_charges_by_customer.failed_charge_amount_this_month, 0)' }} as failed_charge_amount_this_month,
