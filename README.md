...
| **Table** | **Description** | **Example Analytics Questions** |
|-----------|-----------------|-------------------------------|
| [stripe__balance_transactions](https://fivetran.github.io/dbt_stripe/#!/model/model.stripe.stripe__balance_transactions) | Each record represents a change to your account balance, enriched with data about the transaction. | • What types of transactions are most impacting my Stripe balance?<br><br>• How much did fees, refunds, or disputes reduce net revenue this quarter? |
| [stripe__invoice_details](https://fivetran.github.io/dbt_stripe/#!/model/model.stripe.stripe__invoice_details) | Each record represents an invoice, enriched with details about the associated charge, customer, and subscription data. | • What is the average invoice value by customer segment?<br><br>• Which customers have the highest outstanding invoices? |
| [stripe__invoice_line_item_details](https://fivetran.github.io/dbt_stripe/#!/model/model.stripe.stripe__invoice_line_item_details) | Each record represents an invoice line item, enriched with details about the associated charge, customer, subscription, and pricing data. | • Which products or services contribute most to total invoiced revenue?<br><br>• Are there any products consistently discounted or refunded? |
| [stripe__daily_overview](https://fivetran.github.io/dbt_stripe/#!/model/model.stripe.stripe__daily_overview) | Each record represents a daily summary of transaction types (balances, payments, refunds, payouts, etc.) and is useful for MRR reporting. | • What is the trend in daily net payments and refunds?<br><br>• What is the MRR trend over the last 6 months? |
| [stripe__subscription_details](https://fivetran.github.io/dbt_stripe/#!/model/model.stripe.stripe__subscription_details) | Each record represents a subscription, enriched with customer details and payment aggregations. | • How many active subscriptions are there by plan or product?<br><br>• What is the average customer subscription length before cancellation? |
| [stripe__customer_overview](https://fivetran.github.io/dbt_stripe/#!/model/model.stripe.stripe__customer_overview) | Each record represents a customer, enriched with metrics about their transactions. Customers without an associated ID are marked accordingly. | • Who are the top 10 customers by total lifetime value?<br><br>• How many customers made a payment in the last 90 days? |
| [stripe__activity_itemized_2](https://fivetran.github.io/dbt_stripe/#!/model/model.stripe.stripe__activity_itemized_2) | Displays balance transactions with associated customer, charge, refund, fee, and invoice details, especially useful for Interchange Plus (IC+) pricing. | • What are the exact transaction-level fees for each invoice or customer?<br><br>• How much are we paying in interchange and platform fees per transaction? |
| [stripe__balance_change_from_activity_itemized_3](https://fivetran.github.io/dbt_stripe/#!/model/model.stripe.stripe__balance_change_from_activity_itemized_3) | Functions like a bank statement to reconcile your Stripe balance, breaking down all transactions. | • What was the source of each Stripe balance change over the last month?<br><br>• How accurate is my accounting ledger compared to Stripe's balance records? |
| [stripe__ending_balance_reconciliation_itemized_4](https://fivetran.github.io/dbt_stripe/#!/model/model.stripe.stripe__ending_balance_reconciliation_itemized_4) | Matches bank account payouts with underlying transactions and shows unsettled transactions as of the report's end date. | • Which transactions remain unsettled as of the last payout?<br><br>• Do all automatic payouts reconcile fully with balance changes? |
| [stripe__payout_itemized_3](https://fivetran.github.io/dbt_stripe/#!/model/model.stripe.stripe__payout_itemized_3) | Represents payouts with arrival dates and status. Useful for reconciliation and accounting. | • When should I expect my next payout, and for how much?<br><br>• Are there any delayed or failed payouts that need follow-up? |
| [stripe__line_item_enhanced](https://fivetran.github.io/dbt_stripe/#!/model/model.stripe.stripe__line_item_enhanced) | A comprehensive table for subscription, revenue, and customer metrics across billing platforms. Follows the standard `*_line_item_enhanced` schema. | • What are the top revenue-generating products or SKUs?<br><br>• What is the average revenue per user (ARPU) by subscription plan? |
